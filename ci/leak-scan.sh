...
#
# services/dba-mcp is vendored, but it is NOT excluded: vendored or not, it is
# published from this repo, so it has to meet the same bar. Upstream has now had
# the same anonymization pass, so the copy scans clean at the pinned sha. If a
# future re-sync drags an old name back in, this gate is meant to fail — the fix
# is upstream, then re-sync, never an exclusion here.
